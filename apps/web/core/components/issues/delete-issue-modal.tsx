      ...
      await onSubmit()
        ...
        .catch((errors) => {
          const isPermissionError =
            errors?.error ===
            `Only admin or creator can delete the ${isSubIssue ? "sub-ticket" : isEpic ? "epic" : "ticket"}`;
  ...
  return (
    <AlertModalCore
      ...
      content={
        <>
          {/* TODO: Translate here */}
          {`Are you sure you want to delete ${isEpic ? "epic" : "ticket"} `}
          <span className="font-medium break-words text-primary">
            {projectDetails?.identifier}-{issue?.sequence_id}
          </span>
          {` ? All of the data related to the ${isEpic ? "epic" : "ticket"} will be permanently removed. This action cannot be undone.`}
        </>
      }
    />
  );
