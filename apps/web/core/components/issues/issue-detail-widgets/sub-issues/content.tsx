  ...
  const handleFetchSubIssues = useCallback(async () => {
    ...
    if (!currentSubIssueHelpers.issue_visibility.includes(parentIssueId)) {
      try {
        ...
        // Strictly check visibility again after fetch to prevent toggle-off race conditions
        const finalHelpers = subIssueHelpersByIssueId(`${parentIssueId}_root`);
        if (!finalHelpers.issue_visibility.includes(parentIssueId)) {
          setSubIssueHelpers(`${parentIssueId}_root`, "issue_visibility", parentIssueId);
        }
      } catch (error) {
        console.error("Error fetching sub-tickets:", error);
      } finally {
